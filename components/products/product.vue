<template>
  <dl class="s-item" @mouseover="$emit('mouseover', $event)" @mouseenter="handle">
    <dt>
      <img :src="meta.img" alt="商品图片">
    </dt>
    <dd>
      <!-- <h3><nuxt-link :to="{path:'',query:{keyword:meta.name,type:meta.module}}">{{ meta.name }}</nuxt-link></h3> -->
      <h3 >
        <nuxt-link :to="'/detail/'+meta.name+'/'+meta.module.split(';')[0]">{{ meta.name }}</nuxt-link>
      </h3>
